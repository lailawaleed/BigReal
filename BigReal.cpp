...
BigReal BigReal::operator-(BigReal& otherNum) {
    ...
    if (sign != otherNum.sign) {
        BigReal tmp = (absNum1)->plus(*absNum2, true);
...
// Puts the number in the correct format for printing
...
// Overload the insertion operator to prit the Big Real Number
...
// Private Helper Functions That are used by public functions
...
// returns the sign of the Big Real
int BigReal::getSign() {
    // if sign is '-' return -1
    if (sign == '-') {
        return -1;
...
// Returns the size of the Big Real
int BigReal::size() {
    return wholePart.size() + floatPart.size();
}

// Checks if The Big Real is zero or not
// By seeing if all digits are zeros or not 
...
// Takes An number and splits it whole , float and sign 
